...
class Shift(models.Model):
    ...
    def create_auto_shifts(cls, year, month, creation_mode="fill_gaps"):
        ...
        for target_date in target_dates:
            ...
            current_workers = (
                cls.objects.filter(date=target_date, shift_type__is_work=True)
                .exclude(employee__in=available_employees)
                .count()
            )
            ...
            if needed_workers > 0:
                # 勤務可能な従業員から優先的に選択
                work_employees = random.sample(
                    workable_employees, min(needed_workers, len(workable_employees))
                )

                # 勤務可能な従業員が不足する場合、連続勤務日数制限を無視して追加
                if len(work_employees) < needed_workers:
                    remaining_needed = needed_workers - len(work_employees)
                    additional_workers = random.sample(
                        rest_only_employees,
                        min(remaining_needed, len(rest_only_employees)),
                    )
                    work_employees.extend(additional_workers)

            # 残りの従業員
            rest_employees = [
                emp for emp in available_employees if emp not in work_employees
            ]
